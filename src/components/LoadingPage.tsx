...
export function LoadingPage({ 
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-900 flex flex-col justify-between p-4 sm:p-8 relative overflow-hidden font-sans select-none">
      {/* Background Decorative Gradients & Grid */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(16,185,129,0.15),rgba(255,255,255,0))]" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-gradient-to-r from-emerald-600 to-teal-500 shadow-md shadow-emerald-500/20/10 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="relative z-10 flex items-center justify-between w-full max-w-6xl mx-auto">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.2)]">
            <Sprout className="w-5 h-5 text-emerald-600" />
          </div>
          <div>
            <div className="text-base font-bold tracking-tight text-white flex items-center gap-2">
              AgriGPT OS
              <span className="text-[10px] font-semibold bg-emerald-100 text-emerald-300 border border-emerald-200 px-2 py-0.5 rounded-full">
                v2.5 Pro
              </span>
            </div>
            <div className="text-xs text-slate-600">Smart Farming AI Engine</div>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 text-sm font-semibold font-mono text-slate-600 bg-white/80 border border-slate-200/80 px-3 py-1.5 rounded-lg">
          ...
          {standalone && (
            <button
              onClick={() => navigate('/')}
              className="text-sm font-semibold text-slate-600 hover:text-white bg-white/60 hover:bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5"
            >
              Skip to App
      ...
      <div className="relative z-10 w-full max-w-2xl mx-auto my-auto py-8 flex flex-col items-center text-center">
        ...
        <div className="relative mb-8 flex items-center justify-center">
          ...
          <motion.div 
            ...
            className="absolute w-44 h-44 rounded-full bg-emerald-100 blur-xl"
          />
          <motion.div 
            animate={{ rotate: 360 }}
            transition={{ duration: 12, repeat: Infinity, ease: "linear" }}
            className="absolute w-36 h-36 rounded-full border border-dashed border-emerald-200"
          />
          <motion.div 
            animate={{ rotate: -360 }}
            transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
            className="absolute w-28 h-28 rounded-full border border-slate-200 border-t-emerald-400/60"
          />
        ...
        <div className="w-full max-w-md mt-6 space-y-2">
          <div className="flex justify-between items-center text-xs font-mono font-medium text-slate-600 px-0.5">
            <span className="flex items-center gap-1.5 text-emerald-600">
              <Zap className="w-5 h-5 animate-pulse" />
              {isCompleted ? 'System Ready' : 'Processing Neural Weights...'}
            </span>
            <span className="text-slate-800 font-bold text-sm">{progress}%</span>
          </div>

          {/* Progress Bar Container */}
          <div className="h-2.5 w-full bg-white/90 border border-slate-200 rounded-full overflow-hidden p-0.5 relative">
            <motion.div 
        ...
        {isCompleted && (
          ...
          >
            ...
            <button
              onClick={handleReset}
              className="px-6 py-4 min-h-[56px] text-lg min-h-[48px].5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 font-medium text-sm font-semibold rounded-xl flex items-center gap-1.5 transition-colors"
            >
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto mt-auto pt-4">
        <div className="bg-[#121215] border border-slate-200/80 rounded-2xl p-4 sm:p-5 shadow-2xl relative overflow-hidden backdrop-blur-md">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="self-start sm:self-center shrink-0 bg-emerald-50 border border-emerald-500/20 text-emerald-300 text-sm font-semibold font-bold px-3 py-1.5 rounded-lg">
                {currentTip.stat}
              </div>
            </motion.div>
          </AnimatePresence>
        ...
        <div className="text-center text-[11px] text-slate-500 mt-3 flex items-center justify-center gap-2">
