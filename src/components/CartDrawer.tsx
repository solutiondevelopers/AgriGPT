...
export function CartDrawer() {
  ...
  const handlePayment = async () => {
    if (!fullName || !address) {
      setErrorMsg('Please enter delivery details.');
      return;
    }
    ...
    try {
      ...
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          deliveryDetails: { fullName, address }
        })
      });
                  ...
                  <div className="bg-[#18181b] border border-zinc-800 rounded-xl p-4 space-y-3">
                    ...
                    <div className="space-y-2">
                      <input 
                        ...
                        className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500/50" 
                      />
                      <input 
                        type="text" 
                ...
                {checkoutStep === 'cart' ? (
                  ...
                  <>
                    ...
                    <button 
                      ...
                      className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-xl transition-colors font-semibold flex items-center justify-center gap-2"
                    >
                      {isProcessing ? 'Processing...' : `Pay $${(total * 1.05).toFixed(2)}`}
                    </button>
                  </>
                )}
              </div>
